...
export class Generator {
    ...
    private GetInterface(interfaceName: string, inverseInterfaceName: string, codeName: string, transposeCodeName: string, isPlural: boolean): string {
        if (isPlural)
    ...
    GetSchema(): string {
        ...
        edges.forEach(edge => {
            ...
            schema += this.GetInterface(interfaceName, inverseInterfaceName, edge.Name, edge.InverseName, edge.ToCount.trim() != "1");
            schema += this.GetInterface(inverseInterfaceName, interfaceName, edge.InverseName, edge.Name, edge.FromCount.trim() != "1");
            edge.From.forEach(from => {
                edge.To.forEach(to => {
                    if (!typeDict.has(from.Name))
                        typeDict.set(from.Name, []);

                    let a= typeDict.get(from.Name);
                    a!.push(interfaceName);
                    typeDict.set(from.Name, a!);
                    //TODO: Refactor Kill DRY
                    if (!typeDict.has(to.Name))
                        typeDict.set(to.Name, []);
